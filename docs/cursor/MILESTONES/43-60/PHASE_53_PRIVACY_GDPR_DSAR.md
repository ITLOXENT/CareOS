...
- Phase 53 deliverables are implemented.
...
Privacy (GDPR-ready)
- ConsentRecord model: what/when/how + policy version.
- DSAR export:
  - export user/org data to a downloadable artifact (admin authorized).
- DSAR delete:
  - implement 'deactivate/anonymize' where deletion is not lawful; hard-delete only for non-record data.
- Data retention:
  - define retention_class on evidence and key entities; implement scheduled purge hooks (can be Celery stub if already present).
Docs
- Update privacy docs to match actual endpoints and behavior.
