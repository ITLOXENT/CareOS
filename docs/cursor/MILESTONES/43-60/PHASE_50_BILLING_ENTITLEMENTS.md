...
- Phase 50 deliverables are implemented.
...
Backend
- Stripe subscription model (tenant-scoped) with plan/tier, status, current_period_end.
- Entitlements:
  - define features per plan (e.g., max users, evidence storage limit, AI review quota)
  - enforce on API endpoints where relevant (return 402/403 with clear error code)
- Multi-region:
  - support GBP + USD pricing configuration in env/config.
Frontend
- Admin Settings -> Billing: show current plan/status and a placeholder 'Manage billing' link.
