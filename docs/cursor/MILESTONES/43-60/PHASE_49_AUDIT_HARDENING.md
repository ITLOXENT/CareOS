...
- Phase 49 deliverables are implemented.
...
- Ensure audit events for:
  - auth login/logout
  - episode create/read/transition
  - work-item assign/complete
  - evidence upload/download
  - admin settings changes
- Add an export endpoint (admin-only):
  - GET /audit-events?from=&to=&actor=&resource=
- Add tests that assert audit emission for transitions and evidence access.
